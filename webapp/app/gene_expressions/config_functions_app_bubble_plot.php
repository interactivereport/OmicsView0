...
function prepareSingleBubblePlotDataByGeneName_CanvasXpress($geneIndex = -1, $otherOptions = array()){
	...
	if (true){
		...
		$ORDER_ARRAY = array($yAxisColumn => 'ASC');
	...
	foreach($allComparisonData as $tempKey => $comparisonData){
		...
		if ($colorByColumn != ''){
			$results['CanvasXpress']['x'][$colorByColumn][] = '"' . sanitizeJavaScriptValue(ucwords2($currentColorBy)) . '"';
		}
		
		if ($yAxisColumn != ''){
			$results['CanvasXpress']['x'][$yAxisColumn][] 	= '"' . sanitizeJavaScriptValue(ucwords2($currentYAxis)) . '"';
		}
		
		
		if ($subplotColumn != ''){
			$results['CanvasXpress']['x'][$subplotColumn][] = '"' . sanitizeJavaScriptValue(ucwords2($currentSubplot)) . '"';
		}
		
		if ($shapeByColumn != ''){
			$results['CanvasXpress']['x'][$shapeByColumn][] = '"' . sanitizeJavaScriptValue(ucwords2($currentShapeBy)) . '"';
		}
		...
		if ($markerColumn != ''){
			...
			$results['CanvasXpress']['x'][($results['Summary']['Marker'])][] = intval($marker_size);
		}
		

		$results['CanvasXpress']['y']['smps'][] = '"' . "Comparison Data# {$tempKey}" . '"';
		$results['CanvasXpress']['y']['data'][] = $comparisonData['Log2FoldChange'];
	...
	$results['Summary']['Height'] 			= max(800, array_size($results['histogram']['y-axis']) * 20 + 190, array_size($results['histogram']['colorBy']) * 10);

	
	$results['Summary']['y-axis'] 			= $otherOptions['y-axis'];
	$results['Summary']['y-axis_Title'] 	= $APP_CONFIG['DB_Dictionary']['Comparisons']['SQL'][$yAxisColumn]['Title'];
	
	$results['Summary']['colorBy'] 			= $otherOptions['colorBy'];
	$results['Summary']['colorBy_Title'] 	= $APP_CONFIG['DB_Dictionary']['Comparisons']['SQL'][$colorByColumn]['Title'];
